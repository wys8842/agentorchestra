    ...
    def run(self, parameters: Dict[str, Any]) -> ToolResponse:
        ...
        try:
            if mode == "get":
                ...
                rs = self.store.filter(type_name, conds)
                return ToolResponse.success(text=self._fmt(rs), data={"results": rs})
            elif mode == "aggregate":
                ...
                rs = self.store.list_objects(type_name)
                return ToolResponse.success(text=self._fmt(rs), data={"results": rs})
...
class ObjectActionTool(Tool):
    ...
    def run(self, parameters: Dict[str, Any]) -> ToolResponse:
        ...
        if self.security:
            self.security.log(result) if hasattr(self.security, "log") else None
...
class ToolGenerator:
    ...
    def object_query_tool(self, object_type: ObjectType) -> Tool:
        return ObjectQueryTool(object_type, self.store, self.security, self.security_ctx)

    def action_tool(self, action: ActionType) -> Tool:
        return ObjectActionTool(action, self.store, self.security, self.security_ctx)
